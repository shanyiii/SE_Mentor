...
async def get_retrieved_contexts(question: str, chapter: str) -> Tuple[list[str], str]:
    result = await neo4j_retriever(question, chapter)
...
if __name__ == '__main__':
    ...
    result_list = list()
    for data in dataset:
        # contexts, response = asyncio.run(get_retrieved_contexts(data["question"], data["chapter"]))
        ...
        average_score = statistics.fmean([context_precision_score, context_recall_score, faithfulness_score])
        result_list.append({
    ...
    with open("md_files\\JSON\\rag_evaluation_gpt4omini_16.json", 'w', encoding="utf-8") as f:
